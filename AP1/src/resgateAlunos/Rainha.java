...
public class Rainha extends Robo {
	...
	// Construtor para inicializar a classe Rainha
			...
			while(vezes > 0) {
				for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
					for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
						...
						if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Rainha" && PosX >= 2 && PosY >= 2) {
							NovaPosX = PosX-1;
							NovaPosY = PosY-1;
			...
			while(vezes > 0) {
				...
				for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
					for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
						...
						if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Rainha" && PosX < TamX && PosY < TamY) {
							NovaPosX = PosX+1;
							NovaPosY = PosY+1;
							Plano.ListaCelulas.get(i).removerRobo(j);
							Plano.ListaCelulas.get(i).setAndou(true);
