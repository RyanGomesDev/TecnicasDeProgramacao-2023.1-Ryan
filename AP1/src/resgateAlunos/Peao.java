...
public class Peao extends Robo {
	...
	// Construtor para inicializar a classe Peao
	...
	public void avancar(int vezes) {
		...
		for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
			for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
				...
				if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Peao" && PosY < TamY) {
					NovaPosX = PosX;
					NovaPosY = PosY+1;
	...
	public void retroceder(int vezes) {
		...
		for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
			for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
				...
				if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Peao" && PosY > 1) {
					NovaPosX = PosX;
					NovaPosY = PosY-1;
					Plano.ListaCelulas.get(i).removerRobo(j);
					Plano.ListaCelulas.get(i).setAndou(true);
