...
public class Cavalo extends Robo {
	...
	public Cavalo(int id, String nome, Plano plano) {
		super(id, nome, plano);
		Tipo = "Cavalo";
	}
		...
		while(vezes > 0) {
			...
			for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
				for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
					...
					if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Cavalo" && PosX >= 2 && PosY < TamY) {
						NovaPosX = PosX-1;
						NovaPosY = PosY+1;
		...
		while(vezes > 0) {
			...
			for (int i = 0; i < Plano.ListaCelulas.size(); i++) {
				for (int j = 0; j < Plano.ListaCelulas.get(i).getTamanhoRobosCelulas(); j++) {
					...
					if(Plano.ListaCelulas.get(i).getRobo(j).Tipo == "Cavalo" && PosX < TamX && PosY > 1) {
						NovaPosX = PosX+1;
						NovaPosY = PosY-1;
						Plano.ListaCelulas.get(i).removerRobo(j);
						Plano.ListaCelulas.get(i).setAndou(true);
